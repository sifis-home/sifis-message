...
impl Serialize for TryAccessRequest {
        ...
        S: Serializer,
    {
        ...
        match request_kind {
            ...
            RequestKind::GetBrightness => write_request_xml_no_data::<S, _>(
                &mut request_encoder,
                subject,
                resource,
                request::get_brightness,
            )?,
            RequestKind::SetBrightness(value) => write_request_xml_with_data::<S, _, _>(
                &mut request_encoder,
                subject,
                resource,
                value,
...
fn write_request_xml_with_data<'a, S, T, U>(
    writer: impl io::Write,
    subject: &'a str,
    resource: &'a str,
    value: T,
    f: impl Fn(&'a str, &'a str, T) -> request::Request<'a, U>,
) -> Result<(), S::Error>
where
    S: Serializer,
    U: Requestable,
{
    f(subject, resource, value)
        .write_xml(writer)
        .map_err(|err| serde::ser::Error::custom(format!("unable to encode request: {err:?}")))
}
...
mod tests {
    ...
    #[test]
    fn serialize_try_access_request_with_value() {
        use serde_json::Value;
        ...
        let request = TryAccessRequest {
            ...
            request: super::Request {
                subject: "subject".to_owned(),
                resource: "resource".to_owned(),
                kind: super::RequestKind::SetBrightness(42),
            },
        };
        let request_json = serde_json::to_value(request).unwrap();
        assert_eq!(
